...
- `$SUPABASE_DB_URL` is the direct Postgres connection string from Supabase project settings.
...
| Role | Where to find |
|------|--------------|
| Repository owner | GitHub → Find-One repo → Settings → Collaborators |
| Vercel project owner | Vercel dashboard → Find-One → Settings → Members |
| Supabase project owner | Supabase dashboard → Find-One project → Settings → Team |
| GitHub Environments (secrets) | GitHub → Settings → Environments → `production` |
